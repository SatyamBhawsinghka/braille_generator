#!/usr/bin/env python3
# encoding: utf-8
# Inverse kinematics of the 4-DOF manipulator: Given the corresponding coordinates (X, Y, Z)
# and the pitch angle, calculate the rotation angle of each joint
# 2020/07/20 Aiden
import logging
from math import *
...
class IK:
    # Servo counting from bottom to top
    # Common parameters, that is, the link parameters of the 4-DOF manipulator
    l1 = 6.10    #The distance from the center of the robotic arm chassis to the center axis of the second steering gear is 6.10cm
    ...
    l6 = 4.46  #The distance from the top of the suction nozzle to the suction nozzle is 4.46cm
    ...
    def __init__(self, arm_type): #According to different grippers, adaptation parameters
        ...
        if self.arm_type == 'pump': #If it is an air pump type robotic arm
            ...
            self.l4 = 16.65  #The distance from the fourth servo to the end of the robotic arm is 16.6cm,
            # and the end of the robotic arm refers to when the claw is fully closed
    ...
    def getRotationAngle(self, coordinate_data, Alpha):
        ...
        # coordinate_data is the end coordinate of the gripper,
        ...
        # AE is perpendicular to DP_, and E is on DP_, CF is perpendicular to AE, and F is on AE
        # Included angle representation: For example,
        # the included angle between AB and BC is represented as ABC
        ...
        PD = self.l4 * sin(radians(Alpha)) #When the pitch angle is positive,
        # PD is positive, and when the pitch angle is negative, PD is negative
        AF = P_O - CD
        CF = Z - self.l1 - PD
        ...
        if self.l2 + self.l3 < round(AC, 4): #The sum of two sides is less than the third side
            logger.debug('Can not form connecting rod structure, l2(%s) + l3(%s) < AC(%s)', self.l2, self.l3, AC)
            return False

        #ask theta4
        cos_ABC = round(-(pow(AC, 2)- pow(self.l2, 2) - pow(self.l3, 2))/(2*self.l2*self.l3), 4) #cosine law
        if abs(cos_ABC) > 1:
            logger.debug('Can not form connecting rod structure, abs(cos_ABC(%s)) > 1', cos_ABC)
            return False
        ...
        if abs(cos_BAC) > 1:
            logger.debug('Can not form connecting rod structure, abs(cos_BAC(%s)) > 1', cos_BAC)
            return False
        if CF < 0:
            zf_flag = -1
        else:
        ...
        return {"theta3":theta3, "theta4":theta4, "theta5":theta5, "theta6":theta6} # Returns a dictionary of angles when there is a solution
